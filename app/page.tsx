...
export default async function DashboardPage() {
  ...
  const [{ data: salesData }, { data: purchaseData }, { data: receivableData }] = await Promise.all([
    supabase.from("sales_orders").select("order_date,total_amount,is_paid,status,sales_order_items(code,quantity)"),
    supabase.from("purchase_orders").select("status,items:purchase_order_items(code,quantity,unit_price)"),
  ...
  for (const purchase of purchaseOrders) {
    ...
    for (const item of purchase.items || []) {
      ...
      const current = purchaseCostSummary.get(item.code) || { totalCost: 0, totalQty: 0 }
      current.totalCost += safeNumber(item.unit_price) * safeNumber(item.quantity)
      current.totalQty += safeNumber(item.quantity)
      purchaseCostSummary.set(item.code, current)
    }
  }
  ...
  for (const order of monthlySalesOrders) {
    for (const item of order.sales_order_items || []) {
      if (!item.code) continue
      const purchaseCost = purchaseCostSummary.get(item.code)
