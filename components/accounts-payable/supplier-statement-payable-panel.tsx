...
export function SupplierStatementPayablePanel() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { data, error: rpcError } = await supabase.rpc("supplier_statement_payable_period", {
          p_year: year,
          p_month: month,
        })
        if (rpcError) {
  ...
  return (
    <Card className="mb-6">
      <CardHeader>
        <CardTitle>廠商月結應付帳款查詢</CardTitle>
        <CardDescription>根據各廠商月結日自動計算該期應付金額</CardDescription>
      </CardHeader>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b bg-muted">
                  <th className="px-3 py-2 text-left font-semibold">廠商名稱</th>
                  ...
                  <th className="px-3 py-2 text-right font-semibold">本期應付</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row) => (
                  <tr key={row.supplier_id} className="border-b hover:bg-muted/50">
                    ...
                    <td className="px-3 py-2 text-right font-semibold">
                      {(row.total_payable || 0).toLocaleString("zh-TW", {
